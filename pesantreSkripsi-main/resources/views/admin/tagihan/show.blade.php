...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:hidden">
    <div>
        <div class="flex items-center gap-2 text-sm text-surface-500 mb-2">
            <a href="{{ route('admin.tagihan.index') }}" class="hover:text-primary-600 transition-colors">Data Tagihan</a>
    ...
    <div class="flex gap-3">
        ...
        @if($tagihan->status !== 'LUNAS')
            <button onclick="openModalBayar()" class="btn-primary flex items-center gap-2">
    ...
    <div class="lg:col-span-1 space-y-6 print:hidden">
        <x-card title="Riwayat Pembayaran" class="border-t-4 border-t-success-500">
                        ...
                        <div class="p-3 border border-surface-200 rounded-lg hover:bg-surface-50 transition-colors">
                            ...
                            <div class="mt-2 flex justify-between items-center text-xs text-surface-500 border-t border-surface-100 pt-2">
                                ...
                                <div class="flex gap-3 items-center">
                                    <a href="{{ route('admin.pembayaran.kuitansi', $pembayaran) }}" target="_blank" class="text-primary-600 hover:text-primary-800 flex items-center gap-1" title="Cetak Kuitansi">
                                        <i data-lucide="printer" class="w-3.5 h-3.5"></i> Cetak
                                    </a>
                                    <form action="{{ route('admin.pembayaran.destroy', $pembayaran) }}" method="POST" onsubmit="return confirm('Batalkan transaksi ini? Sisa tagihan akan dikalkulasi ulang.');">
                                        @csrf @method('DELETE')
            ...
            <div class="mt-6 space-y-3 pt-4 border-t border-surface-100">
                @if($tagihan->status !== 'LUNAS')
                    <button onclick="openModalBayar()" class="w-full btn-primary flex items-center justify-center gap-2 py-2.5">
                        <i data-lucide="banknote" class="w-4 h-4"></i> Terima Pembayaran
                    </button>

                    {{-- Tombol Kirim Pengingat WA --}}
                    <form action="{{ route('admin.tagihan.wa-reminder', $tagihan) }}" method="POST"
                ...
                @if($tagihan->status === 'BELUM_BAYAR')
                    <form action="{{ route('admin.tagihan.destroy', $tagihan) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan (menghapus) invoice tagihan ini?');">
                        @csrf
                        @method('DELETE')
...
<div id="modal-bayar" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-surface-900/50 backdrop-blur-sm transition-opacity" onclick="closeModalBayar()"></div>
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-visible rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md">
                <div class="px-6 py-4 border-b border-surface-100 flex justify-between items-center bg-surface-50">
                    <h3 class="text-lg font-bold text-surface-900 font-heading">Terima Pembayaran Kasir</h3>
                    <button type="button" onclick="closeModalBayar()" class="text-surface-400 hover:text-surface-600">
                ...
                <form action="{{ route('admin.pembayaran.store') }}" method="POST">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        <div class="bg-primary-50 p-3 rounded-lg border border-primary-100 mb-4 flex justify-between items-center">
                            <span class="text-sm font-medium text-primary-900">Sisa Tagihan:</span>
                            <span class="font-mono font-bold text-xl text-primary-700">Rp {{ number_format($sisaTagihan, 0, ',', '.') }}</span>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-surface-700 mb-1">Nominal Bayar (Rp) <span class="text-danger-500">*</span></label>
                            <input type="number" name="jumlah" required min="1" max="{{ $sisaTagihan }}" value="{{ $sisaTagihan }}" class="w-full font-mono text-lg font-bold rounded-lg border border-surface-300 bg-white px-3 py-2 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-surface-700 mb-1">Metode Pembayaran <span class="text-danger-500">*</span></label>
                            <select name="metode" required class="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-surface-700 mb-1">Keterangan / Catatan (Opsional)</label>
                            <input type="text" name="keterangan" placeholder="Cth: Via Bank BCA, Dititipkan walinya" class="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        </div>
                    </div>
                    
                    <div class="px-6 py-4 bg-surface-50 border-t border-surface-100 flex justify-end gap-3">
                        <button type="button" onclick="closeModalBayar()" class="btn-secondary">Batal</button>
                        <button type="submit" class="btn-primary flex items-center gap-2"><i data-lucide="check" class="w-4 h-4"></i> Proses Pembayaran</button>
                    </div>
                </form>
            </div>
        </div>
