...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    ...
    <div class="flex gap-3">
        <a href="{{ route('admin.psb.calon-santri.index') }}" class="btn-secondary">Kembali</a>
    </div>
</div>
@endsection
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                ...
                <div class="md:col-span-2 border-t border-surface-100 pt-4 mt-2">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Nama</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nama_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">NIK</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nik_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Tahun Lahir</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->tahun_lahir_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pendidikan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pendidikan_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pekerjaan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pekerjaan_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Penghasilan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->penghasilan_ayah ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">No. HP</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->no_hp_ayah ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                <div class="md:col-span-2 border-t border-surface-100 pt-4 mt-2">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Nama</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nama_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">NIK</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nik_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Tahun Lahir</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->tahun_lahir_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pendidikan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pendidikan_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pekerjaan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pekerjaan_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Penghasilan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->penghasilan_ibu ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">No. HP</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->no_hp_ibu ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 border-t border-surface-100 pt-4 mt-2">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Nama</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nama_wali }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Hubungan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->hubungan_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">NIK</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->nik_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Tahun Lahir</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->tahun_lahir_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pendidikan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pendidikan_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Pekerjaan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->pekerjaan_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">Penghasilan</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->penghasilan_wali ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-surface-400 block mb-0.5">No. HP</span>
                            <span class="font-semibold text-surface-900">{{ $calonSantri->no_hp_wali ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                @endif
                    ...
                    @foreach($calonSantri->dokumen as $doc)
                        <div class="border border-surface-200 rounded-xl p-4 flex flex-col items-center justify-between group hover:border-primary-300 hover:shadow-md transition-all bg-white text-center">
                            ...
                            <div class="flex gap-2 w-full mt-2 pt-2 border-t border-surface-100">
                                @if($doc->is_verified)
                                    <form action="{{ route('admin.psb.calon-santri.verifikasi-dokumen', [$calonSantri->id, $doc->id]) }}" method="POST" class="w-full">
                                        @csrf
                                        @method('PATCH')
    ...
    <div class="xl:col-span-1 space-y-6">
        <x-card title="Panel Verifikasi & Keputusan" class="border-t-4 {{ $calonSantri->status === 'DITERIMA' ? 'border-t-success-500' : ($calonSantri->status === 'TIDAK_LULUS' ? 'border-t-danger-500' : ($calonSantri->status === 'HADIR_TES' ? 'border-t-warning-500' : ($calonSantri->status === 'DIBATALKAN' ? 'border-t-surface-400' : 'border-t-primary-500'))) }}">
            ...
            <form action="{{ route('admin.psb.calon-santri.verifikasi', $calonSantri) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
        ...
        @if($calonSantri->status === 'BARU_MASUK')
            <form action="{{ route('admin.psb.calon-santri.destroy', $calonSantri) }}" method="POST" onsubmit="return confirm('Hapus data pendaftar ini selamanya?');">
                @csrf
                @method('DELETE')
