...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-teal-900 via-teal-800 to-teal-950 text-white rounded-3xl p-6 md:p-8 shadow-lg relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-teal-200 text-xs font-semibold backdrop-blur-sm border border-white/10 mb-3">
                ...
                <p class="text-xs md:text-sm text-teal-100/90 mt-1 max-w-xl">
            ...
            <div class="flex items-center gap-3 shrink-0 bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/15">
                ...
                <div>
                    <div class="text-[0.65rem] text-teal-200 uppercase tracking-wider font-semibold">Tahun Pelajaran</div>
                    <div class="text-sm font-bold text-white">{{ $tahunAktif->nama ?? 'Aktif' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white p-5 rounded-2xl border border-surface-200 shadow-sm flex items-center justify-between">
            <div>
                ...
                <div class="text-xl md:text-2xl font-extrabold text-success-700 mt-1">Rp {{ number_format($totalPemasukan, 0, ',', '.') }}</div>
                <div class="text-[0.7rem] text-surface-450 mt-1">Penerimaan riil pembayaran santri</div>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-success-50 text-success-600 flex items-center justify-center shrink-0">
        ...
        <div class="bg-white p-5 rounded-2xl border border-surface-200 shadow-sm flex items-center justify-between">
            ...
            <div class="w-12 h-12 rounded-2xl bg-danger-50 text-danger-600 flex items-center justify-center shrink-0">
        ...
        <div class="bg-white p-5 rounded-2xl border border-surface-200 shadow-sm flex items-center justify-between">
            <div class="flex-1 pr-3">
                ...
                <div class="text-[0.7rem] text-teal-600 font-bold mt-1">{{ $rekapBulanIni['persenLunas'] }}% Terbayar</div>
                <div class="w-full bg-surface-100 rounded-full h-1.5 mt-2 overflow-hidden">
                    <div class="bg-teal-600 h-1.5 rounded-full" style="width: {{ $rekapBulanIni['persenLunas'] }}%"></div>
                </div>
            </div>
    ...
    <div>
        <h2 class="text-base font-bold text-surface-900 mb-3 flex items-center gap-2">
            <i data-lucide="grid" class="w-5 h-5 text-teal-600"></i>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <a href="{{ route('bendahara.tagihan.index') }}" class="group bg-white rounded-3xl p-5 border border-surface-200 shadow-sm hover:shadow-xl hover:border-teal-400 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col justify-between">
                ...
                <div class="relative z-10">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-teal-600 to-teal-800 text-white flex items-center justify-center shadow-md shadow-teal-600/20 mb-4 group-hover:rotate-6 transition-transform">
            ...
            <a href="{{ route('bendahara.laporan-keuangan.index') }}" class="group bg-white rounded-3xl p-5 border border-surface-200 shadow-sm hover:shadow-xl hover:border-emerald-400 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col justify-between">
                ...
                <div class="relative z-10">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-success-700 text-white flex items-center justify-center shadow-md shadow-emerald-500/20 mb-4 group-hover:rotate-6 transition-transform">
            ...
            <form action="{{ route('bendahara.tagihan.blast-reminder') }}" method="POST" class="group bg-white rounded-3xl p-5 border border-surface-200 shadow-sm hover:shadow-xl hover:border-emerald-400 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col justify-between cursor-pointer" onclick="if(confirm('Kirim WhatsApp pengingat tagihan ke seluruh wali santri yang belum lunas?')) this.submit();">
                ...
                <div class="relative z-10">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500 to-emerald-700 text-white flex items-center justify-center shadow-md shadow-green-500/20 mb-4 group-hover:rotate-6 transition-transform">
            ...
            <a href="{{ route('bendahara.komponen-biaya.index') }}" class="group bg-white rounded-3xl p-5 border border-surface-200 shadow-sm hover:shadow-xl hover:border-amber-400 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col justify-between">
                ...
                @forelse($pembayaranTerbaru as $p)
                    <div class="py-3 flex items-center justify-between gap-3 hover:bg-surface-50 p-2 rounded-2xl transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl bg-success-50 text-success-600 flex items-center justify-center shrink-0 font-bold border border-success-100">
                        ...
                        <div class="text-right shrink-0">
                            <span class="text-xs font-mono font-bold text-success-700 block">+ Rp {{ number_format($p->jumlah, 0, ',', '.') }}</span>
                ...
                @forelse($tunggakanJatuhTempo as $t)
                    ...
                    <div class="py-3 flex items-center justify-between gap-3 hover:bg-surface-50 p-2 rounded-2xl transition-colors">
                        ...
                        <div class="text-right flex items-center gap-3 shrink-0">
                            ...
                            <a href="{{ route('bendahara.tagihan.show', $t) }}" class="btn-primary py-1.5 px-3 rounded-xl text-xs font-bold flex items-center gap-1">
                                <span>Bayar</span>
                            </a>
                        </div>
                    </div>
