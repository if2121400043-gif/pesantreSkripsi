...
<body class="min-h-screen bg-surface-50 overflow-x-hidden text-surface-900 font-sans">
    ...
    <a href="#main-content" class="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:top-4 focus:left-4 bg-teal-700 text-white px-4 py-2 rounded-lg font-medium">
        Langsung ke konten
    </a>

    <div class="min-h-screen flex flex-col" id="app-bendahara-layout">
        {{-- Top Bar (Bendahara Brand & User Profile) --}}
        <header class="sticky top-0 z-30 shadow-md border-b border-teal-800" style="background-color: #0f766e !important; color: #ffffff !important;">
            <div class="max-w-screen-xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
                ...
                <a href="{{ route('bendahara.dashboard') }}" class="flex items-center gap-3 group">
                    ...
                    <div>
                        <div class="font-extrabold text-sm sm:text-base font-heading tracking-wide" style="color: #ffffff !important;">PORTAL BENDAHARA</div>
                        <div class="text-[0.65rem] uppercase tracking-wider font-semibold" style="color: #ccfbf1 !important;">PP Nurul Furqon</div>
                    </div>
                </a>

                {{-- Right: User Actions & Switch Role --}}
                <div class="flex items-center gap-3">
                    <a href="{{ route('bendahara.dashboard') }}" class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/15 hover:bg-white/25 text-xs font-bold transition-colors border border-white/20" style="color: #ffffff !important;">
                        <i data-lucide="home" class="w-4 h-4"></i> Beranda
                    </a>

                    {{-- User Dropdown / Role Badge --}}
                    <div class="flex items-center gap-2 pl-2 border-l border-white/20">
                        <a href="{{ route('akun.ganti-peran') }}" title="Ganti Peran / Switch Role" class="px-3 py-1.5 rounded-xl bg-amber-400 text-teal-950 hover:bg-amber-300 text-xs font-extrabold transition-colors border border-amber-300 shadow-sm flex items-center gap-1" style="color: #042f2e !important; background-color: #fbbf24 !important;">
