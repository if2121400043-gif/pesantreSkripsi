...
class TagihanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('bendahara.tagihan.index', compact('tagihans', 'tahuns', 'totalBelumLunas'));
    }
    ...
    public function create()
    {
        ...
        return view('bendahara.tagihan.create', compact('komponenBiayas', 'tahuns', 'lembagas'));
    }
            ...
            foreach ($targetPesertaIds as $pesertaId) {
                ...
                if (!$exists->exists()) {
                    ...
                    if ($phone) {
                        ...
                        ])->delay(now()->addSeconds($waCount * 3));
                        $waCount++;
                    }
                }
            }
            DB::commit();
            return redirect()->route('bendahara.tagihan.index')->with('success', "Berhasil men-generate {$count} tagihan. {$waCount} notifikasi WA sedang dikirim di latar belakang.");
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function show(Tagihan $tagihan)
    {
        ...
        return view('bendahara.tagihan.show', compact('tagihan', 'pembayarans', 'totalDibayar', 'sisaTagihan'));
    }
    ...
    public function bayar(Tagihan $tagihan)
    {
        ...
        if ($sisaTagihan <= 0) {
            return redirect()->route('bendahara.tagihan.show', $tagihan)->with('error', 'Tagihan ini sudah lunas.');
        }
        
        return view('bendahara.tagihan.bayar', compact('tagihan', 'totalDibayar', 'sisaTagihan'));
    }
