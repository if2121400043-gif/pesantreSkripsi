...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <div class="flex items-center gap-2 text-sm text-surface-500 mb-2">
            ...
            <span class="text-surface-900 font-medium">Edit User</span>
        </div>
        <h1 class="text-2xl font-bold text-surface-900 font-heading">Edit User: {{ $user->username }}</h1>
    </div>
...
@section('content')
<form action="{{ route('admin.users.update', $user) }}" method="POST" class="max-w-4xl mx-auto space-y-6">
    @csrf
    @method('PUT')
        ...
        <div class="lg:col-span-2 space-y-6">
            <x-card title="Informasi Akun">
                <div class="space-y-4">
                    ...
                    <x-form-input type="password" name="password" label="Password Baru" placeholder="Kosongkan jika tidak ingin mengubah password" autocomplete="new-password" />
                </div>
            </x-card>
            ...
            <x-card title="Hubungkan Biodata">
                <div class="space-y-4">
                    <div class="space-y-2">
                        ...
                        {{-- Search box: hidden if already has linked orang --}}
                        <div class="relative {{ $user->orang ? 'hidden' : '' }}" id="search_box_container">
                        ...
                        <input type="hidden" name="orang_id" id="orang_id" value="{{ old('orang_id', $user->orang_id) }}">
                        
                        {{-- Selected block: visible if has linked orang --}}
                        <div id="selected_orang" class="{{ $user->orang ? '' : 'hidden' }} flex justify-between items-center bg-primary-50 border border-primary-200 p-3 rounded-lg text-sm">
                            <div>
                                <span class="font-bold text-primary-800" id="selected_orang_name">{{ $user->orang ? $user->orang->nama_lengkap . ' (' . $user->orang->niup . ')' : '' }}</span>
                                <p class="text-xs text-primary-600">Akun ini terhubung dengan biodata di atas.</p>
                            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="pt-4 border-t border-surface-100">
                        <button type="submit" class="btn-primary w-full justify-center flex items-center gap-2 py-3">
                            ...
                            <span class="font-bold">Perbarui User</span>
                        </button>
                    </div>
                </div>
            </x-card>
