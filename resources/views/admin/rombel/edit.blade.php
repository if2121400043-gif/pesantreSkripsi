...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <div class="flex items-center gap-2 text-sm text-surface-500 mb-2">
            ...
            <span class="text-surface-900 font-medium">Edit Kelas</span>
        </div>
        <h1 class="text-2xl font-bold text-surface-900 font-heading">Edit: Kelas {{ $rombel->tingkat }} {{ $rombel->nama_kelas }}</h1>
    </div>
...
@section('content')
<form action="{{ route('admin.rombel.update', $rombel) }}" method="POST" class="max-w-3xl mx-auto space-y-6">
    @csrf
    @method('PUT')
    ...
    <x-card title="Pengaturan Kelas & Lembaga">
        <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="lembaga_id" required class="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        @foreach($lembagas as $lembaga)
                            <option value="{{ $lembaga->id }}" {{ old('lembaga_id', $rombel->lembaga_id) == $lembaga->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="tahun_pelajaran_id" required class="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        @foreach($tahuns as $tahun)
                            <option value="{{ $tahun->id }}" {{ old('tahun_pelajaran_id', $rombel->tahun_pelajaran_id) == $tahun->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-form-input type="text" name="tingkat" label="Tingkat Kelas (Opsional)" placeholder="Contoh: 7, 8, X" value="{{ old('tingkat', $rombel->tingkat) }}" />
                ...
                <div>
                    ...
                    <select name="wali_kelas_id" class="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500">
                        ...
                        @foreach($pegawais as $pegawai)
                            <option value="{{ $pegawai->id }}" {{ old('wali_kelas_id', $rombel->wali_kelas_id) == $pegawai->id ? 'selected' : '' }}>
                                {{ $pegawai->orang->nama_lengkap }} ({{ $pegawai->jenis_pegawai }})
                            </option>
                        @endforeach
                    </select>
                </div>
                <x-form-input type="number" name="kapasitas" label="Kapasitas Siswa Maksimal" required min="1" value="{{ old('kapasitas', $rombel->kapasitas) }}" />
            </div>
        </div>
        
        <div class="mt-6 pt-6 border-t border-surface-100 flex justify-between">
            <button type="button" class="btn-secondary text-danger-600 border-danger-200 hover:bg-danger-50" onclick="document.getElementById('delete-form').submit()">Hapus Kelas</button>
            <div class="flex gap-3">
...
<form id="delete-form" action="{{ route('admin.rombel.destroy', $rombel) }}" method="POST" class="hidden" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kelas ini? Ini akan gagal jika sudah ada siswa di dalamnya.')">
    @csrf
    @method('DELETE')
</form>
@endsection
