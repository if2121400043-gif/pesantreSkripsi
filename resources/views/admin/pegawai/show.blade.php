...
<div class="space-y-6">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 text-xs mt-1" style="color: #a7f3d0 !important;">
                        ...
                        @if($pegawai->is_active)
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[0.68rem] font-bold bg-emerald-100 text-emerald-900">
                                ● Aktif Bekerja
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[0.68rem] font-bold bg-rose-100 text-rose-900">
                                ● Nonaktif
                            </span>
                        @endif
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {{-- Kolom Kiri: Informasi Kepegawaian & Biodata --}}
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-3xl p-6 border border-surface-200 shadow-sm space-y-5">
                ...
                <div class="space-y-4 text-xs">
                    <div>
                        ...
                        <div class="font-bold text-surface-900 text-sm mt-0.5">{{ $pegawai->status_kepegawaian }}</div>
                    </div>
                    <div>
                        <div class="text-surface-500 font-medium">Jabatan Utama</div>
                        <div class="font-bold text-surface-900 text-sm mt-0.5">{{ $pegawai->jabatan ?? 'Staf Operasional' }}</div>
                    </div>
                    <div>
        ...
        {{-- Kolom Kanan: Beban Tugas, Jadwal, & Riwayat --}}
        <div class="lg:col-span-2 space-y-6">
            
            {{-- BEBAN TUGAS CARD --}}
            @if(in_array($pegawai->jenis_pegawai, ['GURU', 'USTADZ', 'PENGASUH']))
                <div class="bg-white rounded-3xl p-6 border border-surface-200 shadow-sm space-y-6">
                                ...
                                <table class="w-full text-xs">
                                    <thead>
                                        <tr class="bg-surface-100 text-surface-600 uppercase tracking-wider font-bold text-[0.65rem]">
                                            ...
                                            <th class="px-4 py-2.5 text-center w-24">Hari</th>
                                                    ...
                                                    <tr class="hover:bg-surface-50 transition-colors">
                                                        ...
                                                        @if($index === 0)
                                                            <td class="px-4 py-2.5 text-center font-bold text-surface-800 bg-surface-50" rowspan="{{ $jadwalGrouped[$hari]->count() }}" style="vertical-align: middle;">
                                                                <span class="inline-block px-2.5 py-1 rounded-lg bg-emerald-700 text-white font-extrabold text-[0.65rem] shadow-2xs" style="color: #ffffff !important; background-color: #047857 !important;">{{ $hari }}</span>
                                                            </td>
                                                        @endif
                                                        <td class="px-4 py-2.5 text-center font-mono font-bold text-emerald-800">
                                                        ...
                                                        <td class="px-4 py-2.5 text-center font-bold text-info-700">
                                                            <span class="px-2.5 py-1 rounded-lg bg-info-50 text-info-800 border border-info-200">{{ $jadwal->rombel->nama ?? '-' }}</span>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @endif
