...
<head>
    ...
    <title>Laporan Presensi - {{ $selectedJadwal->mataPelajaran->nama ?? 'Mapel' }} - {{ $selectedJadwal->rombel->nama ?? 'Kelas' }}</title>
    <style>
        @page {
            size: A4 portrait;
            margin: 1.5cm;
...
<body>
    ...
    <div class="container">
        ...
        <div class="judul-dokumen">
            <h2>LAPORAN REKAPITULASI PRESENSI SANTRI</h2>
        ...
        <table class="meta-table">
            <tr>
                ...
                <td>: {{ $selectedJadwal->mataPelajaran->nama }}</td>
                <td class="label">Hari & Jam Sesi</td>
                <td>: Hari {{ ucfirst(strtolower($selectedJadwal->hari)) }}, {{ substr($selectedJadwal->jam_mulai, 0, 5) }} - {{ substr($selectedJadwal->jam_selesai, 0, 5) }} WITA</td>
            </tr>
            <tr>
                <td class="label">Kelas / Rombel</td>
                <td>: {{ str_starts_with(strtolower($selectedJadwal->rombel->nama ?? ''), 'kelas') ? $selectedJadwal->rombel->nama : 'Kelas ' . ($selectedJadwal->rombel->nama ?? '-') }}</td>
                <td class="label">Guru Pengampu</td>
                <td>: {{ $selectedJadwal->guru->orang->nama_lengkap ?? $pegawai->orang->nama_lengkap ?? '-' }}</td>
            </tr>
        </table>

        {{-- Table Rekap Data Santri --}}
        <table class="data-table">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center">Tidak ada data santri aktif.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <table class="signature-table">
            <tr>
                ...
                <td>
                    ...
                    <strong>Guru Pengampu Mata Pelajaran</strong>
                    <div class="signature-space"></div>
                    <strong><u>{{ $selectedJadwal->guru->orang->nama_lengkap ?? $pegawai->orang->nama_lengkap ?? '-' }}</u></strong>
                </td>
            </tr>
        </table>
