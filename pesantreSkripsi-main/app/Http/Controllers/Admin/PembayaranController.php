...
class PembayaranController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            ...
            // ── Kirim Notifikasi WA: Pembayaran Sukses ──
            try {
                $tagihan->load(['pesertaDidik.orang', 'komponenBiaya']);
                $peserta = $tagihan->pesertaDidik;

                // Gunakan method terpusat di model (termasuk semua fallback)
            ...
            return back()->with('success', 'Pembayaran sebesar Rp ' . number_format($jumlahBayar, 0, ',', '.') . ' berhasil diterima.');
    ...
    public function destroy(Pembayaran $pembayaran)
    {
        ...
        try {
            ...
            DB::rollBack();
            return back()->with('error', 'Terjadi kesalahan saat membatalkan transaksi.');
        }
    }
