...
<x-card :padding="false">
    <div class="p-4 border-b border-surface-100 bg-surface-50">
        <form action="{{ route('admin.psb.gelombang.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="sm:w-64">
            ...
            @if(request()->filled('tahun_pelajaran_id'))
                <a href="{{ route('admin.psb.gelombang.index') }}" class="btn-secondary px-4 py-2 text-danger-600 border-danger-200 hover:bg-danger-50 flex items-center justify-center">
                ...
                <tr class="hover:bg-surface-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        @if($gelombang->calon_santri_count == 0)
                            <form action="{{ route('admin.psb.gelombang.destroy', $gelombang) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus gelombang ini?');">
                                @csrf
                                @method('DELETE')
    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-visible rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-xl">
                ...
                <form id="form-gelombang" action="{{ route('admin.psb.gelombang.store') }}" method="POST">
                    @csrf
...
<script>
    ...
    function openModal() {
        form.reset();
        form.action = "{{ route('admin.psb.gelombang.store') }}";
    ...
    function editGelombang(data) {
        form.action = `/admin/psb/gelombang/${data.id}`;
